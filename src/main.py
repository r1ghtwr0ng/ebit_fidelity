...
def main_switching():
    ...
    # Save the figure
    plt.savefig(f"plots/2d/errbar_plot_simtime_{int(time.time())}.png")
    print("\nPlot saved")
...
def main_heatmap():
    ...
    for i, switch_routing in enumerate(switch_routings):
        ...
        fso_depolar_rates = np.linspace(0, 0.4, 8)
        loss_probabilities = np.linspace(0, 0.4, 8)
        total_runs = 60
        ...
        im = plot_norm_success(
            ax, fso_depolar_rates, loss_probabilities, heatmap_data, titles[i]
        )

    # Add colorbar
    fig.colorbar(im, ax=axs.ravel().tolist(), label="Success probability", shrink=0.6)
    plt.savefig(f"plots/heatmaps/new_fidelity_heatmap_{int(time.time())}.png")
    print("\nPlot saved")
